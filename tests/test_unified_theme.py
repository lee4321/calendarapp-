...
def test_basic_yaml_parses() -> None:
    ...
    for must_have in (
        ...
        "box:duration",
        "box:vline",
        "box:swimlane_heading",
        "box:swimlane_content",
        "box:milestone",
        "line:grid",
        "line:axis",
        "line:today",
...
def test_resolve_token_papersize_override() -> None:
    raw = {
        "style_rules": [
            {"name": "def", "define": "text", "as": "heading", "style": {"font": "Roboto", "size": 10}},
            {
                "name": "small",
...
def test_find_rules_filters_by_target_and_context() -> None:
    raw = {
        "style_rules": [
            {"name": "fed", "apply_to": "box:day", "select": {"federal_holiday": True}, "style": {"fill": "tomato"}},
            {"name": "co", "apply_to": "box:day", "select": {"company_holiday": True}, "style": {"fill": "gold"}},
            {"name": "ev", "apply_to": "box:event", "style": {"fill": "blue"}},
        ]
    }
    theme = parse_theme(raw)
    fed = theme.find_rules("box:day", {"federal_holiday": True})
...
def test_find_rules_multi_target_fans_out() -> None:
    raw = {
        "style_rules": [
            {
                "name": "muted",
                "apply_to": ["box:event", "text:event_name"],
