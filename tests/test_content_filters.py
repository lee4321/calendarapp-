...
_FILTER_FLAGS = frozenset({
    "--empty", "--shade", "--noevents", "--durations", "--nodurations",
    "--milestones", "--includenotes", "--WBS", "--status", "--overflow",
    "--country",
})
...
def _filter_actions(sub: argparse.ArgumentParser) -> dict[str, tuple]:
    ...
    return {
        a.option_strings[0]: (
            tuple(a.option_strings), a.default, a.help, a.metavar, a.type,
            title[id(a)],
        )
...
def test_filters_reach_the_config_identically(command):
    args = _subparsers()[command].parse_args([
        "20260105", "20260109", "--noevents", "--milestones",
        "--WBS", "1.2", "--status", "all", "--country", "GB",
    ])
...
@pytest.mark.parametrize(
    "command", sorted(c for c, flags in _EXPECTED.items() if "--empty" in flags)
)
def test_empty_overrides_the_other_filters(command):
    args = _subparsers()[command].parse_args(
        ["20260105", "20260109", "--empty", "--milestones"]
    )
    args.command = command
    config = create_calendar_config()
    _apply_content_filters(args, config)
...
def test_svg_views_apply_empty_through_the_config_assembly():
    ...
    args = parser.parse_args(
        ["blockplan", "20260105", "20260109", "--empty", "--milestones"]
    )
    config = create_calendar_config()
...
def test_run_wires_the_shared_filters(command, tmp_path, monkeypatch):
    ...
    rc = ecalendar.run([
        "ecalendar.py", command, "20260105", "20260109", "--noevents",
        "--database", str(_REPO_DB), "--quiet",
    ])
